...
app.get('/api/auth/google/callback', async (req, res) => {
    ...
    try {
        ...
        if (!tokens.refresh_token) {
            return res.status(400).send('No refresh token received. If you signed in previously, ' +
                'please disconnect this app from Google account settings and try again.');
        }
        // Encrypt the refresh token statelessly
        const encryptedSession = (0, crypto_1.encrypt)(tokens.refresh_token);
...
app.post('/api/token', async (req, res) => {
    ...
    try {
        const refreshToken = (0, crypto_1.decrypt)(session);
        const client = getOAuth2Client();
